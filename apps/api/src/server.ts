...
const server = new ApolloServer({
  ...
  async context({ req }) {
    ...
    // check if there is a user with the decoded userId - (dont worry about the any typings -- still neeed to setup user models with nexus)
    const user: any = await database.prisma.user.findUnique({
      where: {
        id: userId
      }
    })
    // if there was a matching user and the token was not expired then return the context
    ...
    if (user?.id && Date.now() > exp) {
      ...
      const newToken = jwt.sign({ userId: userId }, `${JWT_SECRET}`, { expiresIn: "2d" })
